...
from powerup import PowerUp  # 补给
...
class AlienInvasion:
    ...
    def __init__(self):
        ...
        self.powerups = pygame.sprite.Group()  # 创建补给编组
    ...
    def _update_screen(self):
        ...
        for powerup in self.powerups.sprites():  # 绘制所有补给
    ...
    def _check_ship_powerup_collisions(self):
        ...
        if collisions:
            for powerup in collisions:
                if powerup.powerup_type == 0:  # 绿色补给 - 增加生命值
                    # 增加生命值
                    self.stats.ships_left += 1
                    self.sb.prep_ships()
                elif powerup.powerup_type == 1:  # 蓝色补给 - 增加子弹宽度
                    # 增加子弹宽度
                    self.settings.bullet_width += self.settings.powerup_bullet_width_bonus

    def _generate_powerup(self):
        """随机生成补给。"""
        if random.randint(0, 10000) < 50:  # 0.5% 概率生成补给
